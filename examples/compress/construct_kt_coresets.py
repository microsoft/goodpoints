...
# When called as a script, call construct_kt_coresets
def main():
    return(construct_kt_coresets(get_args_from_terminal()))


def construct_kt_coresets(args):
    '''

    Generate/load/save KT coresets size 4**args.size/2**args.m from inputs of 
    ...
    delta = 0.5 # (2**args.size)/(2**args.size-1.) * 0.5 # sqrt(n) / (sqrt(n)-1) * delta
